...
void set_up_default_scene() {
  ...
  //ChessBoard
  Point Chess_ctr = {0,0,-3.5};
  float Chess_rad = 2;
  float Chess_ambient[] = {1, 1, 1};
  float Chess_diffuse[] = {1.0, 1.0, 1};
  float Chess_specular[] = {1.0, 1.0, 1.0};
  float Chess_shineness = 10;
  float Chess_reflectance = 0.7;

  scene = add_sphere(scene, Chess_ctr, Chess_rad, Chess_ambient,
             Chess_diffuse, Chess_specular, Chess_shineness,
         Chess_reflectance, 0,false,0,0);

  // sphere 1
  // Point sphere1_ctr = {0, 0, -3};
  ...
  scene = add_sphere(scene, sphere1_ctr, sphere1_rad, sphere1_ambient,
		     ...
		     sphere1_reflectance, 1,false,0,0);
  ...
  scene = add_sphere(scene, sphere2_ctr, sphere2_rad, sphere2_ambient,
		     ...
		     sphere2_reflectance, 2,false,0,0);

  // sphere 3
   Point sphere3_ctr = {-0.35, 1.75, -2.25};
  //  Point sphere3_ctr = {-0.35, 1.75, -3.25};
  ...
  scene = add_sphere(scene, sphere3_ctr, sphere3_rad, sphere3_ambient,
		     ...
		     sphere3_reflectance, 3,false,0,0);
...
void set_up_user_scene() {
  ...
  scene = add_sphere(scene, Chess_ctr, Chess_rad, Chess_ambient,
             Chess_diffuse, Chess_specular, Chess_shineness,
         Chess_reflectance, 0,true,1.3,0);
 ...
 Point sphere1_ctr = {1.5, -0.2, -3.2};
  ...
  scene = add_sphere(scene, sphere1_ctr, sphere1_rad, sphere1_ambient,
             sphere1_diffuse, sphere1_specular, sphere1_shineness,
         sphere1_reflectance, 1,true,1.5,0.3);
  ...
  scene = add_sphere(scene, sphere2_ctr, sphere2_rad, sphere2_ambient,
             sphere2_diffuse, sphere2_specular, sphere2_shineness,
         sphere2_reflectance, 2,true,1.3,0.5);
  ...
  scene = add_sphere(scene, sphere3_ctr, sphere3_rad, sphere3_ambient,
             sphere3_diffuse, sphere3_specular, sphere3_shineness,
         sphere3_reflectance,  3,true,1.2,0.4);
}
